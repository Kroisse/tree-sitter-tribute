...
class Build(build):
    def run(self):
        if path.isdir("queries"):
            dest = path.join(self.build_lib, "tree_sitter_tribute", "queries")
...
setup(
    ...
    package_data={
        "tree_sitter_tribute": ["*.pyi", "py.typed"],
        "tree_sitter_tribute.queries": ["*.scm"],
    },
    ext_package="tree_sitter_tribute",
    ext_modules=[
        Extension(
            name="_binding",
            sources=[
                "bindings/python/tree_sitter_tribute/binding.c",
                "src/parser.c",
            ],
            define_macros=[
